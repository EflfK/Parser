...
{
    public class Power<t1, t2, t3> : BranchingLeaf<t1, t2, t3>
        where t1 : IVariableCollection<t2>
        where t2 : IVariableCollectionKey
        where t3 : IRequiredVariables<t1, t2>
    {
        public Power(String Formula, ILeaf<t1, t2, t3> LHS, ILeaf<t1, t2, t3> RHS)
        ...
        protected override IComparable GetBranchingValue(IComparable LHS, IComparable RHS)
        {
            ...
            if (!double.TryParse(LHS.ToString(), out lhs) ||
                !double.TryParse(RHS.ToString(), out rhs))
                return null;

            return Convert.ToDecimal(Math.Pow(lhs, rhs));
        }
    }
}
